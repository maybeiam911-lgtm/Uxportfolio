import svgPaths from "./svg-1hu2sqrnuc";

function LeftTitleRight() {
  return (
    <div className="content-stretch flex items-start overflow-clip relative shrink-0 w-full" data-name="Left, Title, Right">
      <div className="basis-0 grow h-[44px] min-h-px min-w-px relative shrink-0" data-name="LeftAccessory">
        <div className="flex flex-row items-center size-full">
          <div className="box-border content-stretch flex h-[44px] items-center px-[16px] py-[10px] relative w-full">
            <button className="block cursor-pointer relative shrink-0 size-[28px]" data-name="Icon">
              <div className="absolute bottom-[12.5%] flex items-center justify-center left-1/4 right-[33.33%] top-[12.5%]">
                <div className="flex-none h-[18px] rotate-[180deg] scale-y-[-100%] w-[10px]">
                  <div className="relative size-full" data-name="Icon">
                    <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 10 18">
                      <path clipRule="evenodd" d={svgPaths.p844cd80} fill="var(--fill-0, #8F8F93)" fillRule="evenodd" id="Icon" />
                    </svg>
                  </div>
                </div>
              </div>
            </button>
          </div>
        </div>
      </div>
      <div className="basis-0 grow h-[44px] min-h-px min-w-px relative shrink-0" data-name="RightAccessory">
        <div className="flex flex-row items-center justify-end size-full">
          <div className="h-[44px] w-full" />
        </div>
      </div>
    </div>
  );
}

function NavigationBars() {
  return (
    <div className="absolute bg-white bottom-[82.76%] content-stretch flex flex-col items-start left-0 right-0 top-0" data-name="NavigationBars">
      <div className="box-border content-stretch flex flex-col gap-[10px] items-start pb-[12px] pl-[19px] pr-[15px] pt-[14px] relative shrink-0" data-name="Status bar">
        <div className="h-[18px] relative shrink-0 w-[341px]" data-name="Content">
          <p className="absolute font-['SF_Pro_Text:Semibold',sans-serif] leading-[18px] left-[27px] not-italic text-[15px] text-black text-center top-[calc(50%-9px)] tracking-[-0.165px] translate-x-[-50%] w-[54px]">9:41</p>
          <div className="absolute h-[16px] right-0 top-[calc(50%-1px)] translate-y-[-50%] w-[67px]" data-name="Container">
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 67 16">
              <g clipPath="url(#clip0_1_1429)" id="Container">
                <g id="Battery">
                  <path d={svgPaths.p10163270} fill="var(--fill-0, black)" fillOpacity="0.36" id="Rectangle" />
                  <rect fill="var(--fill-0, black)" height="7.66667" id="Rectangle_2" rx="1.6" width="18" x="45" y="4.07658" />
                </g>
                <path d={svgPaths.p30e84580} fill="var(--fill-0, black)" id="Combined Shape" />
                <path clipRule="evenodd" d={svgPaths.pb4b3500} fill="var(--fill-0, black)" fillRule="evenodd" id="Wi-Fi" />
              </g>
              <defs>
                <clipPath id="clip0_1_1429">
                  <rect fill="white" height="16" width="67" />
                </clipPath>
              </defs>
            </svg>
          </div>
        </div>
      </div>
      <LeftTitleRight />
      <div className="relative shrink-0 w-full" data-name="Title">
        <div className="size-full">
          <div className="box-border content-stretch flex items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
            <p className="basis-0 font-['Inter:Bold',sans-serif] font-bold grow leading-[40px] min-h-px min-w-px not-italic relative shrink-0 text-[#1d1e26] text-[32px]">Title</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function Content() {
  return (
    <div className="absolute bottom-[14.04%] content-stretch flex flex-col items-start left-0 right-0 top-[17.24%]" data-name="Content">
      <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-[375px]" data-name="GroupCell">
        <div className="relative shrink-0 w-full" data-name="Content">
          <div className="size-full">
            <div className="box-border content-stretch flex flex-col items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[12px] shrink-0 w-full" data-name="Extra">
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-[375px]" data-name="GroupCell">
        <div className="bg-white relative shrink-0 w-full" data-name="Header">
          <div className="size-full">
            <div className="box-border content-stretch flex gap-[20px] items-start pb-[8px] pt-[16px] px-[16px] relative w-full">
              <div className="basis-0 flex flex-col font-['Inter:Bold',sans-serif] font-bold grow justify-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 text-[#1d1e26] text-[18px]">
                <p className="leading-[24px]">Title</p>
              </div>
            </div>
          </div>
        </div>
        <div className="relative shrink-0 w-full" data-name="Content">
          <div className="size-full">
            <div className="box-border content-stretch flex flex-col items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[12px] shrink-0 w-full" data-name="Extra">
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-[375px]" data-name="GroupCell">
        <div className="relative shrink-0 w-full" data-name="Content">
          <div className="size-full">
            <div className="box-border content-stretch flex flex-col items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[12px] shrink-0 w-full" data-name="Extra">
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="relative shrink-0 w-full" data-name="GroupLabel">
          <div className="size-full">
            <div className="box-border content-stretch flex items-start pb-[8px] pt-0 px-[16px] relative w-full">
              <p className="basis-0 font-['Inter:Regular',sans-serif] font-normal grow leading-[20px] min-h-px min-w-px not-italic relative shrink-0 text-[#8f8f93] text-[14px]">Description</p>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="GroupCell">
        <div className="bg-white relative shrink-0 w-full" data-name="Header">
          <div className="size-full">
            <div className="box-border content-stretch flex gap-[20px] items-start pb-[8px] pt-[16px] px-[16px] relative w-full">
              <div className="basis-0 flex flex-col font-['Inter:Bold',sans-serif] font-bold grow justify-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 text-[#1d1e26] text-[18px]">
                <p className="leading-[24px]">Title</p>
              </div>
            </div>
          </div>
        </div>
        <div className="relative shrink-0 w-full" data-name="Content">
          <div className="size-full">
            <div className="box-border content-stretch flex flex-col items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[12px] shrink-0 w-full" data-name="Extra">
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="relative shrink-0 w-full" data-name="GroupLabel">
          <div className="size-full">
            <div className="box-border content-stretch flex items-start pb-[8px] pt-0 px-[16px] relative w-full">
              <p className="basis-0 font-['Inter:Regular',sans-serif] font-normal grow leading-[20px] min-h-px min-w-px not-italic relative shrink-0 text-[#8f8f93] text-[14px]">Description</p>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="GroupCell">
        <div className="bg-white relative shrink-0 w-full" data-name="Header">
          <div className="size-full">
            <div className="box-border content-stretch flex gap-[20px] items-start pb-[8px] pt-[16px] px-[16px] relative w-full">
              <div className="basis-0 flex flex-col font-['Inter:Bold',sans-serif] font-bold grow justify-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 text-[#1d1e26] text-[18px]">
                <p className="leading-[24px]">Title</p>
              </div>
            </div>
          </div>
        </div>
        <div className="relative shrink-0 w-full" data-name="Content">
          <div className="size-full">
            <div className="box-border content-stretch flex flex-col items-start pb-[8px] pt-[4px] px-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[12px] shrink-0 w-full" data-name="Extra">
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="bg-white relative shrink-0 w-full" data-name="Empty">
                  <div className="size-full">
                    <div className="box-border content-stretch flex flex-col items-start px-[16px] py-0 relative w-full">
                      <div className="h-[68px] relative shrink-0 w-full" data-name="Place to content">
                        <div className="absolute bg-[#f4f5f9] border border-[rgba(0,0,0,0.5)] border-dashed inset-0 rounded-[6px]" data-name="Place to content" />
                        <div className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[14px] left-[calc(50%+0.25px)] not-italic opacity-50 text-[#1d1e26] text-[12px] text-center text-nowrap top-[calc(50%-14px)] translate-x-[-50%] whitespace-pre">
                          <p className="mb-0">Вставьте что-нибудь</p>
                          <p>c помощью Opt + Cmnd</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default function Component() {
  return (
    <div className="relative size-full" data-name="Component 1">
      <NavigationBars />
      <Content />
      <div className="absolute bg-gradient-to-b bottom-0 content-stretch flex flex-col from-[rgba(255,255,255,0)] items-center left-0 right-0 to-[#ffffff] top-[85.96%]" data-name="BottomBar">
        <div className="relative shrink-0 w-full" data-name="ButtonDiv">
          <div className="size-full">
            <div className="box-border content-stretch cursor-pointer flex flex-col gap-[8px] items-start p-[16px] relative w-full">
              <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Buttons - 3">
                <div className="bg-[#4b4fbf] relative rounded-[12px] shrink-0 w-full" data-name="Content">
                  <div className="flex flex-row items-center justify-center size-full">
                    <div className="box-border content-stretch flex items-center justify-center px-[24px] py-[13px] relative w-full">
                      <p className="font-['Inter:Medium',sans-serif] font-medium leading-[22px] not-italic relative shrink-0 text-[16px] text-center text-nowrap text-white whitespace-pre">Button</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Buttons - 2">
                <div className="bg-[#4b4fbf] relative rounded-[12px] shrink-0 w-full" data-name="Content">
                  <div className="flex flex-row items-center justify-center size-full">
                    <div className="box-border content-stretch flex items-center justify-center px-[24px] py-[13px] relative w-full">
                      <p className="font-['Inter:Medium',sans-serif] font-medium leading-[22px] not-italic relative shrink-0 text-[16px] text-center text-nowrap text-white whitespace-pre">Button</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="content-stretch flex flex-col items-center justify-center relative shrink-0 w-full" data-name="Buttons - 1">
                <div className="bg-[#4b4fbf] relative rounded-[12px] shrink-0 w-full" data-name="Content">
                  <div className="flex flex-row items-center justify-center size-full">
                    <div className="box-border content-stretch flex items-center justify-center px-[24px] py-[13px] relative w-full">
                      <p className="font-['Inter:Medium',sans-serif] font-medium leading-[22px] not-italic relative shrink-0 text-[16px] text-center text-nowrap text-white whitespace-pre">Button</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="h-[34px] relative shrink-0 w-[375px]" data-name="Home indicator">
          <div className="absolute bg-[#1d1e26] bottom-[8px] h-[5px] left-1/2 rounded-[100px] translate-x-[-50%] w-[135px]" data-name="Line" />
        </div>
      </div>
    </div>
  );
}